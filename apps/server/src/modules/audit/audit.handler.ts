...
import type { getAuditByIdRoute, getAuditExportRoute, getAuditPageRoute } from '@server/src/modules/audit/audit.openapi'
...
export const handleGetAuditExport: RouteHandler<typeof getAuditExportRoute> = async (c) => {
  const query = c.req.valid('query')
  const csv = await auditService.exportAuditLogs(query)
