...
import type { authChangePasswordRoute, authLoginRoute, authLogoutRoute, authMenusRoute, authPrefillRoute, authRefreshRoute } from '@server/src/modules/auth/auth.openapi'
...
export const handleAuthChangePassword: RouteHandler<typeof authChangePasswordRoute> = async (c) => {
  const { userId } = getLoginUser(c)
  const body = c.req.valid('json')
