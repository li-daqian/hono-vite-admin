...
import { deleteDepartmentById } from '@admin/client'
...
async function handleDelete() {
  ...
  try {
    await deleteDepartmentById<true>({ path: { id: props.currentRow.id } })
    handleOpenChange(false)
    toast.success(`Department "${props.currentRow.name}" deleted.`)
    emit('success')
  }
  finally {
    ...
    <template #desc>
      <div class="space-y-4">
        <p class="text-sm leading-6">
          ...
          <br>
          This action will permanently remove this department. This cannot be undone.
        </p>
        ...
        <div class="rounded-md border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
          Warning: Departments with child departments or assigned users cannot be deleted.
        </div>
      </div>
    </template>
  </ConfirmDialog>
